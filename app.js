...
function mapColor(n,active=false){if(active)return '#075c57';if(n>=4)return '#d7193f';if(n===3)return '#ff7a21';if(n===2)return '#ffc93c';if(n===1)return '#28a9df';return '#bfe7c6'}
...
function drawThailandMap(geo){let el=$('provinceMap'),w=620,h=650,pad=18,pts=[],activeRows=filtered(),hasFilter=!!(state.province||state.search||state.expertise||state.status||state.duplicate),visibleProvinces=new Set(activeRows.map(r=>r.province));geo.features.forEach(f=>{let g=f.geometry,c=g.type==='Polygon'?[g.coordinates]:g.coordinates;c.forEach(poly=>poly.forEach(r=>r.forEach(p=>pts.push(p))))});let xs=pts.map(p=>p[0]),ys=pts.map(p=>p[1]),minX=Math.min(...xs),maxX=Math.max(...xs),minY=Math.min(...ys),maxY=Math.max(...ys),scale=Math.min((w-2*pad)/(maxX-minX),(h-2*pad)/(maxY-minY)),ox=(w-(maxX-minX)*scale)/2,oy=(h-(maxY-minY)*scale)/2;let project=(x,y)=>[ox+(x-minX)*scale,h-(oy+(y-minY)*scale)];let paths=geo.features.map(f=>{let p=f.properties||{},name=p.ADM1_TH||p.NL_NAME_1||p.name_th||'',n=PSTATS[name]?.dupCount||0,active=state.province===name,visible=!hasFilter||visibleProvinces.has(name);if(!visible)return '';return `<path class="thai-province${active?' selected':''}" data-p="${name}" d="${geoPaths(f.geometry,project)}" fill="${mapColor(n,active)}"><title>${name} • ${n?`ซ้ำซ้อน ${n} สาขา`:'ไม่มีการซ้ำซ้อนของสาขา'} — คลิกเพื่อกรองข้อมูล</title></path>`}).join('');el.innerHTML=`<div class="map-shell"><svg class="thailand-svg" viewBox="0 0 ${w} ${h}" role="img" aria-label="แผนที่ประเทศไทย 77 จังหวัด">${paths}</svg><div class="map-legend"><b>คำอธิบายสี</b><span><i style="background:#bfe7c6"></i>ไม่มีการซ้ำซ้อนของสาขา</span><span><i style="background:#28a9df"></i>ซ้ำซ้อน 1 สาขา</span><span><i style="background:#ffc93c"></i>ซ้ำซ้อน 2 สาขา</span><span><i style="background:#ff7a21"></i>ซ้ำซ้อน 3 สาขา</span><span><i style="background:#d7193f"></i>ซ้ำซ้อนมากกว่า 3 สาขา</span></div><div class="map-selected">${state.province?'จังหวัดที่เลือก: <b>'+state.province+'</b>':'คลิกจังหวัดบนแผนที่เพื่อกรองข้อมูล'}</div></div>`;el.querySelectorAll('.thai-province').forEach(x=>x.onclick=()=>selectProvince(x.dataset.p))}
function renderMap(){
...
function renderDup(rows=DATA){
  ...
  const counts=[0,1,2,3,4].map(k=>entries.filter(x=>k===4?x[1].dupCount>=4:x[1].dupCount===k).length);
  $('dupSummary').innerHTML=[['s0',counts[0],'ไม่มีการซ้ำซ้อน'],['s1',counts[1],'ซ้ำซ้อน 1 สาขา'],['s2',counts[2],'ซ้ำซ้อน 2 สาขา'],['s3',counts[3],'ซ้ำซ้อน 3 สาขา'],['s4',counts[4],'มากกว่า 3 สาขา']].map(x=>`<div class="summary ${x[0]}"><strong>${x[1]}</strong>${x[2]}</div>`).join('');
  $('dupTable').innerHTML=entries.filter(x=>x[1].dupCount>0).sort((a,b)=>b[1].dupCount-a[1].dupCount).map(([p,s])=>`<tr><td>${p}</td><td>ซ้ำซ้อน ${s.dupCount} สาขา</td><td>${s.dup.map(x=>`${x[0].replace('ด้าน','')} (${x[1]})`).join('<br>')}</td></tr>`).join('')||'<tr><td colspan="3">ไม่พบจังหวัดที่มีการซ้ำซ้อนตามตัวกรอง</td></tr>';
}
