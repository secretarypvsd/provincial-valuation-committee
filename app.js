...
function drawThailandMap(geo){let el=$('provinceMap'),w=620,h=650,pad=18,pts=[];geo.features.forEach(f=>{let g=f.geometry,c=g.type==='Polygon'?[g.coordinates]:g.coordinates;c.forEach(poly=>poly.forEach(r=>r.forEach(p=>pts.push(p))))});let xs=pts.map(p=>p[0]),ys=pts.map(p=>p[1]),minX=Math.min(...xs),maxX=Math.max(...xs),minY=Math.min(...ys),maxY=Math.max(...ys),scale=Math.min((w-2*pad)/(maxX-minX),(h-2*pad)/(maxY-minY)),ox=(w-(maxX-minX)*scale)/2,oy=(h-(maxY-minY)*scale)/2;let project=(x,y)=>[ox+(x-minX)*scale,h-(oy+(y-minY)*scale)];let paths=geo.features.map(f=>{let p=f.properties||{},name=p.ADM1_TH||p.NL_NAME_1||p.name_th||'',n=PSTATS[name]?.dupCount||0,active=state.province===name;return `<path class="thai-province${active?' selected':''}" data-p="${name}" d="${geoPaths(f.geometry,project)}" fill="${mapColor(n,active)}"><title>${name} • ${n?`ซ้ำ ${n} สาขา`:'ไม่ซ้ำสาขา'} — คลิกเพื่อกรองข้อมูล</title></path>`}).join('');el.innerHTML=`<div class="map-shell"><svg class="thailand-svg" viewBox="0 0 ${w} ${h}" role="img" aria-label="แผนที่ประเทศไทย 77 จังหวัด">${paths}</svg><div class="map-legend"><b>คำอธิบายสี</b><span><i style="background:#bfe7c6"></i>ไม่ซ้ำสาขา</span><span><i style="background:#28a9df"></i>ซ้ำ 1 สาขา</span><span><i style="background:#ffc93c"></i>ซ้ำ 2 สาขา</span><span><i style="background:#ff7a21"></i>ซ้ำ 3 สาขา</span><span><i style="background:#d7193f"></i>ซ้ำมากกว่า 3 สาขา</span></div><div class="map-selected">${state.province?'จังหวัดที่เลือก: <b>'+state.province+'</b>':'คลิกจังหวัดบนแผนที่เพื่อกรองข้อมูล'}</div></div>`;el.querySelectorAll('.thai-province').forEach(x=>x.onclick=()=>selectProvince(x.dataset.p))}
function renderMap(){
