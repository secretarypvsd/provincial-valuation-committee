...
function renderDup(rows=DATA){
  ...
  $('dupTable').innerHTML=entries.filter(x=>x[1].dupCount>0).sort((a,b)=>b[1].dupCount-a[1].dupCount).map(([p,s])=>`<tr><td>${p}</td><td>ซ้ำ ${s.dupCount} ราย</td><td>${s.dup.map(x=>`${x[0].replace('ด้าน','')} (${x[1]} คน)`).join('<br>')}</td></tr>`).join('')||'<tr><td colspan="3">ไม่พบจังหวัดที่มีผู้ทรงคุณวุฒิซ้ำสาขาตามตัวกรอง</td></tr>';
}
